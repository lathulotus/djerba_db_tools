"""
Use couchDB python library to apply specified mango queries based on input filters to output JSONs for reports satisfying filters (or report counts)
...
plugin_format = {
    ...
    "views": {
        "snv_genes": {
            "map": """function(doc) {
                try {
                    if (doc.plugins && doc.plugins["wgts.snv_indel"]) {
                        var arr = doc.plugins["wgts.snv_indel"].results.Body;
                        if (Array.isArray(arr)) {
                            arr.forEach(function(item) {
                                emit([item.Gene, item.type], doc._id);
                            });
                        }
                    }
                } catch(e) {}
        ...
        "cnv_genes": {
            "map": """function(doc) {
                try {
                    if (doc.plugins && doc.plugins["wgts.cnv_purple"]) {
                        var arr = doc.plugins["wgts.cnv_purple"].results.body;
                        if (Array.isArray(arr)) {
                            arr.forEach(function(item) {
                                emit([item.Gene, item.Alteration], doc._id);
                            });
                        }
                    }
                } catch(e) {}
...
def main():
    ...
    try:
        ...
        while True:
            paged_query = query.copy()
            paged_query["limit"] = args.page_size
            paged_query["skip"] = skip
            ...
            for doc in results:
                candidate_ids.add(doc["_id"])
            skip += len(results)
        final_ids = candidate_ids.copy()

        # SNV filter
        if filters.get("snv"):
            snv_ids = set()
            for snv_single in filters["snv"]:
                parts = snv_single.split(" ", 1)
                gene = parts[0]
                mutation_type = parts[1] if len(parts) > 1 else None
                snv_ids |= query_snv(db, gene, mutation_type)
            final_ids &= snv_ids

        # CNV filter
        if filters.get("cnv"):
            cnv_ids = set()
            for cnv_single in filters["cnv"]:
                parts = cnv_single.split(" ", 1)
                gene = parts[0]
                mutation_type = parts[1] if len(parts) > 1 else None
                cnv_ids |= query_cnv(db, gene, mutation_type)
            final_ids &= cnv_ids
        
        query = {"selector": {"_id": {"$in": list(final_ids)}}}
