""" Cleaned summary tables counting number of cases by given parameter

Availabe parameters: OncoTree code, SNV, CNV, PGA, Fusion, TMB, MSI status, HRD score, Purity

Usage:
    python3 distribution_table.py --summary_csv input
 """
import os
import argparse
import pandas as pd
...
with pd.ExcelWriter(out("summary_clean.xlsx")) as writer:
    ...
    if onco.empty: onco = pd.Series({"Blank": 0})
    combined["oncotree"] = write_table(onco, "OncoTree_Code", out("oncotree_count.csv"), writer, "oncotree_count")

    # 2) SNV
    snv = explode_and_clean(df["snv_genes"]).value_counts()
    if snv.empty: snv = pd.Series({"Blank": 0})
    combined["snv"] = write_table(snv, "SNV_Gene", out("snv_count.csv"), writer, "snv_count")

    # 3) CNV
    genes = explode_and_clean(df["cnv_genes"])
    types = explode_and_clean(df["cnv_types"])
    ...
    if amps.empty: amps = pd.Series({"Blank": 0})
    if dels.empty: dels = pd.Series({"Blank": 0})
    ...
    pga_hist = make_histogram_df(pga, np.arange(0, 110, 10), "Percentage(%)")
    pga_hist.to_excel(writer, sheet_name="pga", index=False)
    pga_hist.to_csv(out("pga_histogram.csv"), index=False)
    combined["pga"] = pga_hist

    # 5) Fusions
    fus = explode_and_clean(df["fusion_pairs"]).value_counts()
    if fus.empty: fus = pd.Series({"Blank": 0})
    combined["fusions"] = write_table(fus, "Fusion_Pair", out("fusion_count.csv"), writer, "fusions")

    # 6) TMB
    tmb = pd.to_numeric(df["TMB"], errors="coerce")
    tmb_hist = make_histogram_df(tmb, np.arange(0, 51, 1), "TMB_Range(Mb)")
    tmb_hist.to_excel(writer, sheet_name="tmb", index=False)
    tmb_hist.to_csv(out("tmb_histogram.csv"), index=False)
