...
def main():
    ...
    plt.bar(merged["project"], merged["low_purity_fails"], label="Low-purity fails", color="#234c13ff")
    plt.bar(merged["project"], merged["other_reports"], bottom=merged["low_purity_fails"], label="Total reports", color="#7cb066ff")
    plt.xlabel("project")
    plt.ylabel("Report Count")
    plt.title("Low-Purity Failures by Project")
